...
class AprilaireThermostatCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.config = {
      ...
      // Real Feel configuration
      real_feel_target: config.real_feel_target || 'input_number.target_feels_like_temperature',
      real_feel_sensor: config.real_feel_sensor || 'sensor.thermal_comfort_heat_index',
      real_feel_override: config.real_feel_override || 'input_boolean.smart_comfort_override',
      real_feel_min: config.real_feel_min || 68,
      real_feel_max: config.real_feel_max || 78,
      real_feel_step: config.real_feel_step || 1,
      ...config
    };
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .realfeel-current {
          ...
          font-weight: 200;
          color: #ffcc66;
          text-shadow: 0 2px 12px rgba(255,200,100,0.4);
        }
        ...
        .realfeel-target {
          ...
          font-weight: 200;
          color: #ffaa44;
          text-shadow: 0 2px 8px rgba(255,170,68,0.3);
        }
        ...
        .side-btn.realfeel {
          background: linear-gradient(145deg, rgba(255, 170, 68, 0.4), rgba(200, 130, 50, 0.3));
          border-color: rgba(255, 170, 68, 0.35);
        }
        
        .side-btn.realfeel:hover {
          background: linear-gradient(145deg, rgba(255, 170, 68, 0.5), rgba(200, 130, 50, 0.4));
        }
        
        .side-btn.realfeel.active {
          background: linear-gradient(145deg, rgba(255, 170, 68, 0.6), rgba(200, 130, 50, 0.5));
          border-color: rgba(255, 170, 68, 0.7);
          box-shadow: 0 4px 16px rgba(255, 170, 68, 0.3), inset 0 1px 0 rgba(255,255,255,0.2);
        }
        
        .smart-comfort-toggle-container {
          margin-top: 8px;
        }
        ...
        .smart-comfort-btn:hover {
          background: linear-gradient(145deg, rgba(30,90,125,0.5), rgba(20,60,90,0.4));
          transform: scale(1.02);
        }
        
        .smart-comfort-btn.active {
          background: linear-gradient(145deg, rgba(74, 222, 128, 0.4), rgba(34, 197, 94, 0.3));
          border-color: rgba(74, 222, 128, 0.5);
        }
        
        .smart-comfort-indicator {
          width: 36px;
          height: 20px;
          background: rgba(255,255,255,0.2);
          border-radius: 10px;
        ...
        .smart-comfort-btn.active .smart-comfort-indicator {
          background: rgba(74, 222, 128, 0.6);
        }
        
        .smart-comfort-indicator::after {
          content: '';
          position: absolute;
          top: 2px;
          left: 2px;
        ...
        .smart-comfort-btn.active .smart-comfort-indicator::after {
          left: 18px;
        }
      ...
      <div class="card-container">
        ...
        <div class="main-content">
          <div class="temp-section">
            ...
            <div class="smart-comfort-toggle-container" id="smart-comfort-container">
              <button class="smart-comfort-btn" id="smart-comfort-btn">
                <span class="smart-comfort-indicator" id="smart-comfort-indicator"></span>
                <span class="smart-comfort-label" id="smart-comfort-label">Smart Comfort OFF</span>
              </button>
            </div>
          </div>
          ...
          <div class="controls view-hidden" id="realfeel-view">
            <div class="realfeel-control">
              <div class="realfeel-label">Current Feels Like</div>
              <div class="realfeel-current"><span id="realfeel-current">--</span>°</div>
              <div class="realfeel-target-section">
                <div class="realfeel-label">Target Feels Like</div>
                <div class="realfeel-target"><span id="realfeel-target">--</span>°</div>
                <div class="realfeel-buttons">
                  <button class="temp-btn" id="realfeel-up-btn">▲</button>
                  <button class="temp-btn" id="realfeel-down-btn">▼</button>
                </div>
              </div>
            </div>
          </div>
  ...
  setupEventListeners() {
    ...
    // Real Feel adjustment buttons
    const realfeelUpBtn = this.shadowRoot.getElementById('realfeel-up-btn');
    const realfeelDownBtn = this.shadowRoot.getElementById('realfeel-down-btn');
    const smartComfortBtn = this.shadowRoot.getElementById('smart-comfort-btn');
    
    if (realfeelUpBtn) realfeelUpBtn.addEventListener('click', () => this.adjustRealFeel(this.config.real_feel_step));
    if (realfeelDownBtn) realfeelDownBtn.addEventListener('click', () => this.adjustRealFeel(-this.config.real_feel_step));
    if (smartComfortBtn) smartComfortBtn.addEventListener('click', () => this.toggleSmartComfort());
  }
  ...
  renderSideControls() {
    ...
    if (thermostats[0]) {
      const isSelected = this.activeEntity === thermostats[0].entity;
      const thermoEntity = this._hass ? this._hass.states[thermostats[0].entity] : null;
      let statusText = 'Off';
      if (thermoEntity) {
        const hvacAction = thermoEntity.attributes.hvac_action;
        const hvacMode = thermoEntity.state;
        if (hvacAction === 'heating') {
          statusText = '🔥 Heating';
        } else if (hvacAction === 'cooling') {
          statusText = '❄️ Cooling';
        } else if (hvacAction === 'idle') {
          statusText = 'Idle';
        } else if (hvacMode === 'off') {
          statusText = 'Off';
        } else {
          statusText = hvacMode.charAt(0).toUpperCase() + hvacMode.slice(1).replace('_', ' ');
        }
      }
      html += `
        <button class="side-btn thermostat-3 ${isSelected ? 'active' : ''}" id="thermostat-btn-0">
          <span class="side-btn-icon">🌡️</span>
          <span class="side-btn-text">${thermostats[0].name || 'Upstairs Thermostat'}<br/><span class="side-btn-label" id="thermostat-0-status">${statusText}</span></span>
        </button>
      `;
    }
    ...
    // 2.5. Real Feel control
    if (this.config.real_feel_target) {
      const isRealFeelActive = this.viewMode === 'realfeel';
      html += `
        <button class="side-btn realfeel ${isRealFeelActive ? 'active' : ''}" id="realfeel-btn">
          <span class="side-btn-icon">🌡️</span>
          <span class="side-btn-text">Real Feel<br/><span class="side-btn-label" id="realfeel-status-label">Control</span></span>
        </button>
      `;
    }
    ...
    if (thermostats[1]) {
      const isSelected = this.activeEntity === thermostats[1].entity;
      const thermoEntity = this._hass ? this._hass.states[thermostats[1].entity] : null;
      let statusText = 'Off';
      if (thermoEntity) {
        const hvacAction = thermoEntity.attributes.hvac_action;
        const hvacMode = thermoEntity.state;
      ...
      html += `
        <button class="side-btn thermostat-2 ${isSelected ? 'active' : ''}" id="thermostat-btn-1">
          <span class="side-btn-icon">🌡️</span>
          <span class="side-btn-text">${thermostats[1].name || 'Downstairs Thermostat'}<br/><span class="side-btn-label" id="thermostat-1-status">${statusText}</span></span>
        </button>
      `;
    }
    ...
    // Real Feel button listener
    if (this.config.real_feel_target) {
      const realfeelBtn = this.shadowRoot.getElementById('realfeel-btn');
      if (realfeelBtn) {
        realfeelBtn.addEventListener('click', () => this.toggleRealFeel());
      }
    }
  ...
  update() {
    ...
    this.updateSideButtonStatuses();
  }

  updateSideButtonStatuses() {
    if (!this._hass) return;
    
    // Collect thermostats
    const thermostats = this.config.thermostats.filter(t => t.entity.startsWith('climate.'));
    
    // Update first thermostat status
    if (thermostats[0]) {
      const statusEl = this.shadowRoot.getElementById('thermostat-0-status');
      if (statusEl) {
        const thermoEntity = this._hass.states[thermostats[0].entity];
        if (thermoEntity) {
          const hvacAction = thermoEntity.attributes.hvac_action;
          const hvacMode = thermoEntity.state;
          let statusText = 'Off';
    ...
    // Update second thermostat status
    if (thermostats[1]) {
      const statusEl = this.shadowRoot.getElementById('thermostat-1-status');
      if (statusEl) {
        const thermoEntity = this._hass.states[thermostats[1].entity];
        if (thermoEntity) {
          const hvacAction = thermoEntity.attributes.hvac_action;
          const hvacMode = thermoEntity.state;
          let statusText = 'Off';
  ...
  toggleWeather() {
    ...
    const realfeelView = this.shadowRoot.getElementById('realfeel-view');
    const weatherBtn = this.shadowRoot.getElementById('weather-toggle-btn');
    ...
    if (this.viewMode === 'weather') {
      ...
      if (realfeelView) realfeelView.classList.add('view-hidden');
    ...
    // Update side control button states
    this.renderSideControls();
  }
  ...
  toggleRealFeel() {
    console.log('Real Feel toggle clicked, current mode:', this.viewMode);
    ...
    if (this.viewMode === 'realfeel') {
      // Switch back to thermostat
      console.log('Switching back to thermostat from realfeel');
      this.viewMode = 'thermostat';
      if (weatherView) weatherView.classList.add('view-hidden');
      if (thermostatControls) thermostatControls.classList.remove('view-hidden');
      if (dehumidifierView) dehumidifierView.classList.add('view-hidden');
      ...
      // Switch to real feel view
      console.log('Switching to realfeel view');
      this.viewMode = 'realfeel';
      if (weatherView) weatherView.classList.add('view-hidden');
      if (thermostatControls) thermostatControls.classList.add('view-hidden');
      if (dehumidifierView) dehumidifierView.classList.add('view-hidden');
      if (realfeelView) realfeelView.classList.remove('view-hidden');
      if (weatherBtn) weatherBtn.classList.remove('active');
      this.updateRealFeel();
    }
  ...
  adjustRealFeel(delta) {
    console.log('adjustRealFeel called with delta:', delta);
    
    const targetEntity = this._hass.states[this.config.real_feel_target];
    if (!targetEntity) {
      console.error('Real feel target entity not found:', this.config.real_feel_target);
      return;
    }
    
    const currentTarget = parseFloat(targetEntity.state) || 73;
    const newTarget = Math.max(
      this.config.real_feel_min, 
      Math.min(this.config.real_feel_max, currentTarget + delta)
    );
    
    console.log('Adjusting real feel target:', {
      entity_id: this.config.real_feel_target,
      current: currentTarget,
      new: newTarget,
      min: this.config.real_feel_min,
      max: this.config.real_feel_max,
      delta: delta
    });
    ...
    const targetEl = this.shadowRoot.getElementById('realfeel-target');
    if (targetEl) {
      targetEl.textContent = Math.round(newTarget);
    }
  }

  toggleSmartComfort() {
    console.log('Toggle smart comfort clicked');
    
    const overrideEntity = this._hass.states[this.config.real_feel_override];
    if (!overrideEntity) {
      console.error('Real feel override entity not found:', this.config.real_feel_override);
      return;
    }
    
    // Toggle the input_boolean
    this._hass.callService('input_boolean', 'toggle', {
      entity_id: this.config.real_feel_override
    });
    
    // Update display after a short delay
    setTimeout(() => this.updateRealFeel(), 500);
  }

  updateRealFeel() {
    if (!this.config.real_feel_target) return;
    
    const targetEntity = this._hass.states[this.config.real_feel_target];
    const sensorEntity = this._hass.states[this.config.real_feel_sensor];
    const overrideEntity = this._hass.states[this.config.real_feel_override];
    
    // Update side button status label
    const statusLabelEl = this.shadowRoot.getElementById('realfeel-status-label');
    const realfeelBtn = this.shadowRoot.getElementById('realfeel-btn');
    
    if (statusLabelEl && overrideEntity) {
      // ON means smart comfort is ACTIVE
      const isActive = overrideEntity.state === 'on';
      statusLabelEl.textContent = isActive ? 'Active' : 'OFF';
    }
    
    if (realfeelBtn) {
      realfeelBtn.classList.toggle('active', this.viewMode === 'realfeel');
    }
    
    // Update smart comfort button state (always visible now)
    const smartComfortContainer = this.shadowRoot.getElementById('smart-comfort-container');
    const smartComfortBtn = this.shadowRoot.getElementById('smart-comfort-btn');
    const smartComfortLabel = this.shadowRoot.getElementById('smart-comfort-label');
    
    // Update smart comfort button state
    if (smartComfortBtn && smartComfortLabel && overrideEntity) {
      const isActive = overrideEntity.state === 'on';
      smartComfortBtn.classList.toggle('active', isActive);
      smartComfortLabel.textContent = isActive ? 'Smart Comfort ON' : 'Smart Comfort OFF';
    }
    
    // Update the real feel view if it's visible
    if (this.viewMode === 'realfeel') {
      const currentEl = this.shadowRoot.getElementById('realfeel-current');
      const targetEl = this.shadowRoot.getElementById('realfeel-target');
      const locationLabel = this.shadowRoot.getElementById('location-label');
      
      if (currentEl && sensorEntity) {
        currentEl.textContent = Math.round(parseFloat(sensorEntity.state));
      }
      
      if (targetEl && targetEntity) {
        targetEl.textContent = Math.round(parseFloat(targetEntity.state));
      }
      
      if (locationLabel) {
        locationLabel.textContent = 'Real Feel';
      }
      
      // Update the left panel to show real feel info
      const humidityEl = this.shadowRoot.getElementById('humidity');
      if (humidityEl && targetEntity) {
        humidityEl.textContent = `Target: ${Math.round(parseFloat(targetEntity.state))}°`;
      }
    }
  }
}
...
window.customCards.push({
  ...
  description: 'A custom card for comprehensive climate control with Real Feel adjustment'
});
